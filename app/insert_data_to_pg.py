...
def func_insert_stock_data_into_table(table_data, conn):
    ...
    insert_query = """
        INSERT INTO ita_table (
            created_at, code, pdpp, pdv, pprp, pdop, pdhp, pdlp, pvwap, pqap, pqas, pqbp, pqbs, paav, pabv,
        ...
        VALUES (
            %(created_at)s, %(sIssueCode)s, %(pDPP)s, %(pDV)s, %(pPRP)s, %(pDOP)s, %(pDHP)s, %(pDLP)s, %(pVWAP)s, %(pQAP)s,
    ...
    try:
        with conn.cursor() as cursor:
            # 空白のNone埋め
            timestamp_str = table_data["p_rv_date"].replace(" ", "")
            dt = datetime.strptime(timestamp_str, '%Y.%m.%d-%H:%M:%S.%f')
            data_to_insert = [
                convert_empty_string_to_none({**item, "created_at": dt.strftime('%Y-%m-%d %H:%M:%S.%f')})
